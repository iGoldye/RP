...
function updateMarkersAndBlips()
    ...
    if Config.MaxPatientsPerTrip <= #Overlay.gameData.pedsInAmbulance or #coordsList == 0 then
        SetNewWaypoint(gameData.hospitalLocation.x, gameData.hospitalLocation.y)
    elseif #coordsList > 0 then
        SetNewWaypoint(coordsList[1].x, coordsList[1].y)
    end
