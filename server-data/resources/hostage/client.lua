...
function takeHostage()
	...
	if not holdingHostageInProgress and canTakeHostage then
		...
		if closestPlayer ~= nil then
			...
			drawNativeNotification("Рядом никого нет!")
		end
	end
...
Citizen.CreateThread(function()
	while true do
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"[F] отпустить, [K] убить")
			if IsDisabledControlJustPressed(0,23) then --release
				--print("release this mofo")
				holdingHostage = false
				holdingHostageInProgress = false
				local closestPlayer = GetClosestPlayer(2)
...
function releaseHostage()
	...
	if closestPlayer ~= nil then
		...
		print("[CMG Anim] Рядом никого нет!")
	end
end
