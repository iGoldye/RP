        ...
        if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
            if distanceToDSDF <= 3 then
                ...
                elseif GetVehicleDoorAngleRatio(vehicle, 0) <= 0.1 then
                    chatPrint('Откройте дверь прежде чем брать оружие')
                end
            end
        else
            chatPrint('Вы не коп')
...
AddEventHandler('startT', function(ello, we, rem, source)
    ...
    --         Citizen.Wait(0)
    --         if remove ~= true then
    --             Draw3DText(coordsMe['x'], coordsMe['y'], coordsMe['z'] - 0.0, "Вы достаёте " .. weapon .. " из патрульного автомобиля")
    --         elseif remove == true then
    --             Draw3DText(coordsMe['x'], coordsMe['y'], coordsMe['z'] - 0.0, "Вы ложите " .. weapon .. " в патрульный автомобиль")
    --         end
    --     end
    -- end)
end)
...
function isPoliceCar(vehicle)
	...
	elseif model == GetHashKey("lapd9") then
		return true
	elseif model == GetHashKey("sp106") then
        return true
    elseif model == GetHashKey("fibjacal") then
		return true
	end
...
function chatPrint( msg )
	TriggerEvent( 'chatMessage', "Оповещение: ", { 255, 255, 255 }, msg )
end
