...
on('attachPropRadio', (attachModel, boneNumber, x, y, z, xR, yR, zR) => {
  ...
  let HasModelLoadedLoop = setInterval(() => {HasModelLoadedLoopFunction()}, 10);
...
on('animation:radio', async (enable) => {
  ...
  let HasAnimDictLoadedLoop = setInterval(() => { HasAnimDictLoadedLoopFunction() }, 10 );
...
on("__cfx_nui:close", (data, cb) => {
  ...
  if(newChannel === 0) {
    emitNet('TokoVoip:removePlayerFromAllRadio', playerServerId);
	} else {
