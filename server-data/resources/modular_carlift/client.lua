...
function Main()
	...
	Menu.addOption("mainmenu", function() if (Menu.Option("Включить подьемник")) then
		spawnProp("nacelle", elevatorBaseX, elevatorBaseY, elevatorBaseZ)
	end end)
	
	Menu.addOption("mainmenu", function() if (Menu.Option("Поднять подьемник")) then
    ...
    Menu.addOption("mainmenu", function() if (Menu.Option("Опустить подьемник")) then
		if elevatorProp ~= nil then
			elevatorUp = false
			elevatorDown = true
		end
