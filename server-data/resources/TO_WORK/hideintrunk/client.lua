	...
	Citizen.CreateThread(function()
	  	while true do
		    ...
		    if IsDisabledControlJustReleased(0, 20) and GetVehiclePedIsIn(player, false) == 0 and DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) then
		    	...
		    	if not inside then
		        	AttachEntityToEntity(player, vehicle, -1, 0.0, -2.2, 0.5, 0.0, 0.0, 0.0, false, false, false, false, 20, true)
		       		...
		       		if IsEntityAttached(player) then
						...
						AddTextComponentString('~n~~s~~INPUT_HUD_SPECIAL~ Выйлезти')
		    	...
		    	elseif inside and IsDisabledControlJustReleased(0, 20) then
	    	...
	    	if DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) and not inside and GetVehiclePedIsIn(player, false) == 0 then
						SetTextComponentFormat("STRING")
						AddTextComponentString('~n~~s~~INPUT_HUD_SPECIAL~ Залезть в багажник')
...
function VehicleInFront()
    ...
    local entityWorld = GetOffsetFromEntityInWorldCoords(player, 0.0, 1.0, 0.0)
