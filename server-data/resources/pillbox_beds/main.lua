	...
	while true do
		...
		if CurrentBed ~= nil then
			if not OnBed then
				...
				Draw3DText({x = BedCoords.x, y = BedCoords.y, z = (BedCoords.z+1)}, 'Нажмите ~g~[E]~w~ что бы лечь', 0.35)
			end
		end
	end
end)
	...
	while true do
		...
		if CurrentBed ~= nil then
			if OnBed then
				...
				Draw3DText({x = BedCoords.x, y = BedCoords.y, z = (BedCoords.z+1)}, 'Нажмите ~g~[Q]~w~ что бы встать', 0.35)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentBed ~= nil and IsControlJustReleased(0, 38) then
			...
			OnBed = true
		elseif IsControlJustReleased(0, 44) and IsEntityPlayingAnim(PlayerPedId(), 'missfbi1', 'cpr_pumpchest_idle', 3) then
