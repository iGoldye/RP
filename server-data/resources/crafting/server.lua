...
RegisterServerEvent('crafting:getPlayerInventory')
AddEventHandler('crafting:getPlayerInventory', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer ~= nil then
		TriggerClientEvent('crafting:openMenu', _source, xPlayer.inventory)
	end
end)
...
AddEventHandler('crafting:craftItem', function(ingredients)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	...
	if not item then
		...
		if xPlayer ~= nil then
			if hasAllIngredients(xPlayer.inventory, Config.Recipes[item]) then
				...
				TriggerClientEvent('esx:showNotification', _source, '~y~Вы сделали: ~w~' .. itemLabel(item, xPlayer.inventory))
			else
				TriggerClientEvent('esx:showNotification', _source, 'Вам не хватает ингридиентов')
			end
		end
	end
end)
