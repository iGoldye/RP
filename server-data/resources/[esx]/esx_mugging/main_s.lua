...
AddEventHandler('esx_mugging:giveMoney', function()
    ...
    TriggerClientEvent("esx:showNotification", source, ("Вы украли $%s"):format(amount))
end)
...
AddEventHandler('esx_mugging:muggingAlert', function(street1, street2, sex, coords)
    if Config.GCPhone  then
        TriggerClientEvent("muggingNotify", -1, "Сообщение об ограблении "..sex.." возле "..street1.." и "..street2, coords)
    else
	    TriggerClientEvent("muggingNotify", -1, "~r~Сообщение об ограблении ~w~ "..sex.." ~r~возле~w~ "..street1.." и "..street2, coords)
    end
end)
...
AddEventHandler('esx_mugging:muggingAlertS1', function(street1, sex, coords)
    if Config.GCPhone  then
        TriggerClientEvent("muggingNotify", -1, "RСообщение об ограблении "..sex.." возле "..street1, coords)
    else
	    TriggerClientEvent("muggingNotify", -1, "~r~Сообщение об ограблении~w~ "..sex.." ~r~возле~w~ "..street1, coords)
    end
end)
RegisterServerEvent('esx_mugging:muggingAlertS2')
AddEventHandler('esx_mugging:muggingAlertS2', function(street1, sex, coords)
    if Config.GCPhone then
        TriggerClientEvent("muggingNotify", -1, "Сообщение об ограблении "..sex.." near "..street1, coords)
    else
        TriggerClientEvent("muggingNotify", -1, "~r~Сообщение об ограблении~w~ "..sex.." ~r~возле~w~ "..street1, coords)
    end
end)
