...
function robNpc(targetPed)
    ...
    ESX.Game.Utils.DrawText3D(roblocalcoords, "[~g~E~s~] Ограбить", 0.25)
    elseif lasttargetPed == targetPed then
    ESX.Game.Utils.DrawText3D(roblocalcoords, "Уже ограблен..", 0.25)
    else
        if not Config.progressBars then
        ESX.Game.Utils.DrawText3D(roblocalcoords, "Грабеж..", 0.25)
        end
    end
...
AddEventHandler('muggingNotify', function(alert, xPlayer, coords)
        if  ESX.PlayerData.job.name == 'police' then
            if Config.GCPhone then
                ...
                ESX.ShowAdvancedNotification('911', 'Ограбление', alert, 'CHAR_CALL911', 1)
            end
        end
end)
