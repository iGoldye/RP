...
AddEventHandler('esx_SodaMachine:Random', function(prop_name)
	...
	if lottery <= 6 then
		if not IsAnimated then
			...
			Citizen.CreateThread(function()
				...
				Citizen.Wait(700)
				ESX.ShowNotification('Вы купили банку ~r~Газировки~s~ за ~g~$1~s~. Спасибо за покупку!')
	...
	if lottery == 7 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('Автомат не работает')
			end)
		end
	if lottery == 8 then
			Citizen.CreateThread(function()
			ESX.ShowNotification('В автомате кончилась газировка')
			end)
		end
	if lottery == 9 then
			Citizen.CreateThread(function()
			...
			TriggerEvent('InteractSound_CL:PlayOnOne', 'SodaMachine', 1.0)
			Citizen.Wait(5000)
			ESX.ShowNotification('В автомате застряли ваши деньги.')
			FreezeEntityPosition(ped, false)
			end)
		end
end)