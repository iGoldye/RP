	...
	while true do
		...
		if IsControlPressed(0, Keys['LEFTALT']) and IsControlPressed(0, Keys['E']) and not Aresztuje and GetGameTimer() - OstatnioAresztowany > 10 * 1000 and PlayerData.job.name == PracaPolice then	-- Mozesz tutaj zmienic przyciski
			...
			if distance ~= -1 and distance <= Config.ArrestDistance and not Aresztuje and not Aresztowany and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				ESX.ShowNotification("~b~Вы арестовываете гражданина~r~ ") --.. GetPlayerServerId(closestPlayer) .. "")						-- Drukuje Notyfikacje
				...
				ESX.ShowNotification("~b~Гражданин арестован ~r~ ") --.. GetPlayerServerId(closestPlayer) .. "")					-- Drukuje Notyfikacje
