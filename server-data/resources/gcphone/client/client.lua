...
function showFixePhoneHelper (coords)
  for number, data in pairs(FixePhone) do
    ...
    if dist <= 2.0 then
      ...
      AddTextComponentString("~g~" .. data.name .. ' ~o~' .. number .. '~n~~INPUT_PICKUP~~w~ Использовать')
  ...
  while true do
    ...
    for i, _ in pairs(PhoneInCall) do
        ...
        if (dist <= soundDistanceMax) then
          ...
          if (dist <= 1.5) then
            SetTextComponentFormat("STRING")
            AddTextComponentString("~INPUT_PICKUP~ Поднять трубку")
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    ...
    if ShowNumberNotification == true then
      text = '~o~Новое сообщение: ~y~'.. message.transmitter
      for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
          text = '~o~Новое сообщение: ~g~'.. contact.display
          break
        end
      end
    end
