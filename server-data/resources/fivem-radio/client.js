...
for (let i = 0, length = GetNumResourceMetadata("fivem-radio", "supersede_radio"); i < length; i++) {
    const radio = GetResourceMetadata("fivem-radio", "supersede_radio", i);
    ...
    try {
        const data = JSON.parse(GetResourceMetadata("fivem-radio", "supersede_radio_extra", i));
        if (data !== null) {
            customRadios.push({
                "isPlaying": false,
                "name": radio,
