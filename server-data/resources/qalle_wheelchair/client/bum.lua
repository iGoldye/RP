...
Citizen.CreateThread(function()
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			local bumcartCoords = GetEntityCoords(closestObject)
			local bumcartForward = GetEntityForwardVector(closestObject)
...
Sit = function(bumcartObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'missfinale_c2leadinoutfin_c_int', '_leadin_loop2_lester', 3) then
			ShowNotification("Somebody is already using the bumcart!")
			return
		end
	end
	...
	AttachEntityToEntity(PlayerPedId(), bumcartObject, 0, 0, 0.0, 0.4, 0.0, 0.0, 180.0, 0.0, false, false, false, false, 2, true)

	local heading = GetEntityHeading(bumcartObject)

	while IsEntityAttachedToEntity(PlayerPedId(), bumcartObject) do
		Citizen.Wait(5)
		...
		if IsControlPressed(0, 32) then
			local x, y, z  = table.unpack(GetEntityCoords(bumcartObject) + GetEntityForwardVector(bumcartObject) * -0.02)
			SetEntityCoords(bumcartObject, x,y,z)
			PlaceObjectOnGroundProperly(bumcartObject)
		end
		...
		if IsControlPressed(1,  34) then
			...
			SetEntityHeading(bumcartObject,  heading)
		end
		...
		if IsControlJustPressed(0, 73) then
			...
			local x, y, z = table.unpack(GetEntityCoords(bumcartObject) + GetEntityForwardVector(bumcartObject) * - 0.7)
...
PickUp = function(bumcartObject)
	local closestPlayer, closestPlayerDist = GetClosestPlayer()

	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'anim@heists@box_carry@', 'idle', 3) then
			ShowNotification("Somebody is already driving the bumcart!")
			return
		end
	end

	NetworkRequestControlOfEntity(bumcartObject)

	LoadAnim("anim@heists@box_carry@")

	AttachEntityToEntity(bumcartObject, PlayerPedId(), GetPedBoneIndex(PlayerPedId(),  28422), -0.00, -0.3, -0.73, 195.0, 180.0, 180.0, 0.0, false, false, true, false, 2, true)

	while IsEntityAttachedToEntity(bumcartObject, PlayerPedId()) do
		Citizen.Wait(5)
		...
		if IsPedDeadOrDying(PlayerPedId()) then
			DetachEntity(bumcartObject, true, true)
		end
